  ...
  const handleGoogleLogin = async () => {
    ...
    const { error: signInError } =
      await supabase.auth.signInWithOAuth({
        ...
        options: {
          redirectTo:
            `${window.location.origin}/auth?portal=rider`,
          queryParams: {
      ...
      <div className="auth-form-card w-full max-w-[25rem] overflow-hidden rounded-[1.6rem]">
        <div className="relative overflow-hidden bg-[#211914] px-6 pb-7 pt-6 text-white sm:px-8">
          ...
          <div className="relative mt-7">
            ...
            <p className="mt-1.5 text-xs leading-relaxed text-white/60">
              Sign in to receive delivery requests,
              manage active deliveries, and update
              delivery progress.
            </p>
          </div>
        </div>
        ...
        <div className="bg-card/90 p-6 sm:p-8">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-[10px] leading-relaxed text-muted-foreground">
                  Rider access is available only to
                  registered and approved RRJ's
                  Food-Haus delivery partners.
                </p>
              </div>
            </div>
          </div>
          ...
          <p className="mt-4 text-center text-[10px] leading-relaxed text-muted-foreground">
            Need rider access? Contact RRJ's Food-Haus
            management.
          </p>
        </div>
      </div>
